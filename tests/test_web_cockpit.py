...
class WorkbenchCockpitSmokeTests(unittest.TestCase):
    ...
    def test_cockpit_renderers_are_wired_into_home(self) -> None:
        required_functions = [
            ...
            "workspaceCockpitActionsFromNextAction",
        ...
        self.assertIn('if (root) root.innerHTML = "";', self.app)
        ...
        self.assertIn("class=\"workspace-manage-advanced-tabs\"", self.html)
        ...
        self.assertIn('class="workspace-execution-inspector-chat"', self.html)
        ...
        self.assertIn("workspace-node-summary-more", self.app)
        self.assertIn("节点诊断", self.app)
        ...
        self.assertIn('layout: "monitor"', self.app)
        self.assertIn("workspaceUseMonitorMarkup(workspace", self.app)
        self.assertIn("runs.innerHTML = workspaceHomeRunsMarkup(workspace);", self.app)
        self.assertIn("cockpit.innerHTML = \"\";", self.app)
        self.assertIn("执行包 / 复现包", self.app)
        self.assertIn("function workspaceExecutionPackageSummaryMarkup", self.app)
        ...
        home_decision_index = self.app.index('layout: "monitor"', home_render_index)
        home_command_model_index = self.app.index("function workspaceCommandCenterMarkup")
        home_runs_index = self.app.index("runs.innerHTML = workspaceHomeRunsMarkup(workspace);", home_render_index)
        self.assertLess(home_command_model_index, home_render_index)
        self.assertLess(home_decision_index, home_runs_index)
        cockpit_overview_index = self.app.index("function renderWorkspaceCockpitOverview")
        overview_decision_index = self.app.index("renderWorkspaceUseMonitor(", cockpit_overview_index)
        ...
        self.assertIn("topology.innerHTML = \"\";", self.app)
        self.assertIn("接管摘要", self.app)
        self.assertIn("当前接管焦点", self.app)
        self.assertIn("分层接管明细", self.app)
        self.assertIn("runs.innerHTML = workspaceHomeRunsMarkup(workspace);", self.app)
        self.assertIn("function workspaceExecutionRuns(", self.app)
        self.assertIn("function workspaceExecutionRunItemMarkup(", self.app)
        self.assertIn("workspaceRunProgressMarkup(latestRun", self.app)
        self.assertIn("runs.slice(0, 12).map((run) => workspaceExecutionRunItemMarkup(run))", self.app)
        ...
        self.assertIn("context.innerHTML = \"\";", self.app)
        ...
        self.assertIn("resourcesPanel.innerHTML = \"\";", self.app)
        ...
        self.assertIn("chainRoot.innerHTML = workspaceCockpitChainMarkup(workspace, template, { focusNodeId });", self.app)
        self.assertIn('if (action === "focus-workspace-execution-board")', self.app)
        self.assertIn(".workspace-cockpit-chain-node", self.css)
        self.assertIn("const cockpitNext = workspaceCockpitNextAction(workspace);", self.app)
        self.assertIn("if (cockpitNext?.primary) {", self.app)
        self.assertIn("return workspaceCommandCenterModelFromCockpit(cockpitNext, workspace, options);", self.app)
        self.assertIn("workspaceCockpitActionsFromNextAction(cockpitNext)", self.app)
        self.assertIn("focusWorkspaceCockpitFromNextAction(selectedWorkspace());", self.app)
        self.assertIn('parsed.path.endswith("/cockpit")', (ROOT / "total_control" / "server.py").read_text(encoding="utf-8"))
        ...
        self.assertIn("flow.innerHTML = \"\";", self.app)
        self.assertIn("执行链责任矩阵", self.app)
        self.assertIn("function workspaceHomeResponsibilityMatrixMarkup", self.app)
        ...
        self.assertIn("function workspaceNodeIoContractMarkup", self.app)
        self.assertIn("当前节点执行计划", self.app)
        self.assertIn("function workspaceNodeExecutionPlanMarkup", self.app)
        ...
        self.assertIn("refreshWorkspaceExecutionFlowCanvas(box);", self.app)
