...
class WorkbenchCockpitSmokeTests(unittest.TestCase):
    ...
    def test_cockpit_home_has_required_mount_points(self) -> None:
        required_ids = [
            ...
            "workspaceCockpitPipeline",
            "workspaceCockpitHandoff",
            "workspaceCockpitLayering",
            "workspaceCockpitScheduling",
            "workspaceCockpitBlueprint",
            "workspaceCockpitChain",
            "workspaceCockpitGapQueue",
    ...
    def test_file_picker_supports_quick_preview_cache_actions(self) -> None:
        ...
        self.assertIn("点文件可快览并缓存到本机", self.app)
    ...
    def test_cockpit_renderers_are_wired_into_home(self) -> None:
        ...
        self.assertIn('id="workspaceLauncherDetails" class="workspace-launcher-details"', self.html)
        self.assertIn('id="workspaceCapabilityBaseline" class="workspace-capability-baseline"', self.html)
        self.assertIn('class="workspace-home-collapse workspace-use-support-collapse"', self.html)
        self.assertIn('class="workspace-home-collapse workspace-launcher-details-collapse"', self.html)
        self.assertIn('class="workspace-use-secondary-stack"', self.html)
        self.assertIn("补充线索（可选）", self.html)
        self.assertIn("启动包预览", self.html)
        self.assertIn('const detailsRoot = $("workspaceLauncherDetails");', self.app)
        self.assertNotIn('if (root) root.innerHTML = "";', self.app)
        self.assertIn("root.innerHTML = workspaceLauncherPlanMarkup(inputs, template, summary, { compact: true });", self.app)
        self.assertIn("detailsRoot.innerHTML = workspaceLauncherDetailsMarkup(inputs, template, summary);", self.app)
        self.assertIn("capabilityBaseline.innerHTML = workspaceCapabilityBaselineMarkup", self.app)
        self.assertIn("renderWorkspaceLauncherPlan(inputs, template, resources);", self.app)
        self.assertIn('"workspaceLauncherPlan"', self.app)
        self.assertIn('"workspaceLauncherDetails"', self.app)
        self.assertIn('"workspaceCapabilityBaseline"', self.app)
        self.assertIn(".workspace-use-cockpit-shell {\n  display: grid;", self.css)
        self.assertIn("grid-template-columns: 1fr;\n  gap: 8px;", self.css)
        self.assertIn(".workspace-use-status-panel .workspace-launcher-plan-shell {\n  grid-template-columns: 1fr;", self.css)
        self.assertIn(".workspace-use-status-panel .workspace-home-decision,\n.workspace-use-status-panel .workspace-capability-baseline {\n  display: none;", self.css)
        self.assertIn(".workspace-use-status-panel .workspace-launch-start-summary {\n  grid-template-columns: minmax(180px, 0.28fr) minmax(0, 1fr);", self.css)
        ...
        self.assertIn(".workspace-use-status-panel .workspace-launch-start-summary {\n    grid-template-columns: 1fr;", self.css)
        self.assertIn('class="workspace-use-launcher-grid"', self.html)
        self.assertEqual(self.html.count('id="workspaceCreateRunTaskBtn"'), 1)
        self.assertIn('class="workspace-use-drawer workspace-use-chat"', self.html)
        self.assertIn('class="workspace-use-drawer workspace-use-history"', self.html)
        ...
        self.assertIn(".workspace-use-launcher-grid .workspace-launch-start-focus {\n  display: none;", self.css)
        self.assertIn("@media (min-width: 1081px) and (max-width: 1359px) {", self.css)
        self.assertIn(".workspace-use-launcher-grid {\n    grid-template-columns: minmax(0, 1fr);", self.css)
        self.assertIn("@media (max-width: 1359px) {\n  .workspace-execution-board {\n    grid-template-columns: 1fr;", self.css)
        self.assertIn("const showDetails = options.showDetails === true;", self.app)
        self.assertIn("showDetails && details", self.app)
        self.assertIn("const showBlockers = blockers.length && (workspace?.id || options.showBlockers === true);", self.app)
        ...
        self.assertIn(".workspace-use-template-band .subsection-head {\n  display: none;", self.css)
        self.assertIn(".workspace-use-template-band .workspace-template-summary-text {\n  display: none;", self.css)
        ...
        self.assertIn("${workspaceDetailClosureMarkup(null, node, sourceNode, { template })}", self.app)
        self.assertIn("${workspaceDetailClosureMarkup(workspace, node, sourceNode)}", self.app)
        ...
        self.assertIn("#workspaceLauncherDetails [data-action=\"select-execution-node\"]", self.app)
        self.assertIn("function workspaceDetailActionsMarkup", self.app)
        self.assertIn('class="workspace-detail-drawer"', self.app)
        self.assertIn("节点明细与诊断", self.app)
        self.assertIn("模板节点明细", self.app)
        ...
        self.assertIn('id="workspaceProjectQueue" class="workspace-project-queue"', self.html)
        self.assertIn("实例队列", self.html)
        self.assertIn("实例明细", self.html)
        self.assertIn("workspaceProjectQueueMarkup(items)", self.app)
        self.assertIn("完整列表在下方明细里展开", self.app)
        self.assertIn('"workspaceProjectQueue"', self.app)
        self.assertIn('id="workspaceWorkflowSummary" class="workspace-workflow-summary"', self.html)
        self.assertIn("renderWorkspaceWorkflowSummary();", self.app)
        self.assertIn("执行链摘要", self.app)
        self.assertIn("当前节点焦点", self.app)
        self.assertIn("工作流页默认先解释链路", self.app)
        self.assertIn('"workspaceWorkflowSummary"', self.app)
        self.assertIn('id="workspaceAgentCoverageSummary" class="workspace-agent-coverage-summary"', self.html)
        self.assertIn('id="workspaceAgentAdvancedDetails" class="workspace-home-collapse workspace-panel-details"', self.html)
        self.assertIn("Agent 高级配置", self.html)
        self.assertIn("workspaceAgentCoverageSummaryMarkup()", self.app)
        self.assertIn("Agent 接管摘要", self.app)
        self.assertIn("当前 Agent 焦点", self.app)
        self.assertIn('"merge-recommended-workspace-agents"', self.app)
        self.assertIn('"select-workspace-agent"', self.app)
        self.assertIn('id="workspaceToolAdvancedDetails" class="workspace-home-collapse workspace-panel-details"', self.html)
        self.assertIn("工具覆盖摘要", self.app)
        self.assertIn('"merge-recommended-workspace-tools"', self.app)
        self.assertIn('id="workspaceModelRoutingSummary" class="workspace-model-routing-summary"', self.html)
        self.assertIn('id="workspaceModelAdvancedDetails" class="workspace-home-collapse workspace-panel-details"', self.html)
        self.assertIn("workspaceModelRoutingSummaryMarkup()", self.app)
        self.assertIn("AI 路由摘要", self.app)
        self.assertIn('"open-workspace-details"', self.app)
        self.assertIn("const activeRevealTarget = {", self.app)
        self.assertIn('model: $("workspaceModelRoutingSummary")', self.app)
        self.assertIn("revealWorkspaceElement(activeRevealTarget", self.app)
        self.assertIn(".workspace-project-modules:not(.home-active) .workspace-module-band", self.css)
        self.assertIn("workspaceHomeCockpitMarkup(workspace, formData)", self.app)
        self.assertIn("执行包 / 复现包", self.app)
        self.assertIn("workspaceExecutionPackageSummaryMarkup(workspace, { compact: true })", self.app)
        self.assertIn('"workspaceCockpitCards"', self.app)
        self.assertIn("decision.innerHTML = workspaceCommandCenterMarkup", self.app)
        self.assertIn("decisionRoot.innerHTML = workspaceCommandCenterMarkup", self.app)
        self.assertIn("const advancedDetails = `", self.app)
        self.assertIn("${workspaceAutomationDecisionMarkup(workspace)}", self.app)
        self.assertIn("${workspaceAutomationPlaybookMarkup(workspace, { compact: true })}", self.app)
        self.assertIn("workspaceAutomationPlaybookMarkup(workspace, { compact: true })", self.app)
        self.assertIn("当前阶段", self.app)
        self.assertIn("下一步", self.app)
        self.assertIn("卡点", self.app)
        self.assertIn("无硬阻塞", self.app)
        self.assertIn("workspaceCommandCenterMainlineMarkup(model, blockers)", self.app)
        self.assertIn(".workspace-command-center-mainline", self.css)
        self.assertIn(".workspace-command-center-mainline-card", self.css)
        self.assertIn("目标与细则", self.app)
        self.assertIn("工作台默认服务于自动复现/部署", self.app)
        self.assertIn("高级详情", self.app)
        self.assertIn('class="workspace-home-collapse workspace-home-status-details"', self.html)
        self.assertIn('class="workspace-home-collapse workspace-home-advanced-section"', self.html)
        self.assertIn('class="workspace-home-collapse workspace-cockpit-advanced"', self.html)
        self.assertIn("启动包与预检细节", self.html)
        self.assertIn("门禁、资源和分层", self.html)
        self.assertIn("Agent 分工矩阵", self.app)
        home_render_index = self.app.index("function renderWorkspaceHome")
        home_advanced_index = self.app.index("const advancedDetails = `", home_render_index)
        home_decision_index = self.app.index("decision.innerHTML = workspaceCommandCenterMarkup", home_render_index)
        home_command_model_index = self.app.index("function workspaceCommandCenterMarkup")
        home_playbook_index = self.app.index("${workspaceAutomationPlaybookMarkup(workspace, { compact: true })}", home_advanced_index)
        home_cockpit_index = self.app.index("cockpit.innerHTML = workspaceHomeCockpitMarkup(workspace, formData);", home_render_index)
        home_package_index = self.app.index("${workspaceExecutionPackageSummaryMarkup(workspace, { compact: true })}", home_advanced_index)
        self.assertLess(home_command_model_index, home_render_index)
        self.assertLess(home_advanced_index, home_decision_index)
        self.assertLess(home_decision_index, home_cockpit_index)
        self.assertLess(home_playbook_index, home_cockpit_index)
        self.assertLess(home_package_index, home_cockpit_index)
        overview_advanced_index = self.app.index("const advancedDetails = `", cockpit_overview_index)
        overview_decision_index = self.app.index("decisionRoot.innerHTML = workspaceCommandCenterMarkup", cockpit_overview_index)
        overview_playbook_index = self.app.index("${workspaceAutomationPlaybookMarkup(workspace, { compact: true })}", overview_advanced_index)
        overview_package_index = self.app.index("${workspaceExecutionPackageSummaryMarkup(workspace, { compact: true })}", overview_advanced_index)
        overview_cards_index = self.app.index("const jobs = workspace ? workspaceJobs() : [];", cockpit_overview_index)
        self.assertLess(overview_advanced_index, overview_decision_index)
        self.assertLess(overview_decision_index, overview_cards_index)
        self.assertLess(overview_playbook_index, overview_cards_index)
        self.assertLess(overview_package_index, overview_cards_index)
        self.assertIn('"workspaceCockpitDecision"', self.app)
        self.assertIn('"workspaceHomeDecision"', self.app)
        self.assertIn("const visibleSteps = steps;", self.app)
        self.assertIn("自动推进决策", self.app)
        self.assertIn("playbook.current_action", self.app)
        self.assertIn("automation.execution_readiness", self.app)
        self.assertIn("automation.resource_orchestration", self.app)
        self.assertIn("automation.dataset_discovery", self.app)
        self.assertIn("workspace-automation-decision-actions", self.app)
        self.assertIn("blueprintRoot.innerHTML = workspaceRuntimeBlueprintMarkup(workspace, formData, resources);", self.app)
        self.assertIn("const pipelineRoot = $(\"workspaceCockpitPipeline\");", self.app)
        self.assertIn("pipelineRoot.innerHTML = workspaceReproductionPipelineMarkup(workspace, formData, resources, template);", self.app)
        self.assertIn("const handoffRoot = $(\"workspaceCockpitHandoff\");", self.app)
        self.assertIn("handoffRoot.innerHTML = workspaceCockpitHandoffMapMarkup(workspace, template);", self.app)
        self.assertIn("const layeringRoot = $(\"workspaceCockpitLayering\");", self.app)
        self.assertIn("layeringRoot.innerHTML = workspaceAgentLayerBusMarkup(workspace, template, { limit: 8 });", self.app)
        self.assertIn('id="workspaceCockpitLayering" class="workspace-cockpit-layering"', self.html)
        self.assertIn("Agent / Tool / AI 接管总线", self.app)
        self.assertIn("topology.innerHTML = workspaceHomeLayeringMarkup(workspace, selectedWorkflowTemplate());", self.app)
        self.assertIn("接管摘要", self.app)
        self.assertIn("当前接管焦点", self.app)
        self.assertIn("分层接管明细", self.app)
        self.assertIn("runs.innerHTML = workspaceHomeRunsMarkup(workspace);", self.app)
        self.assertIn("运行摘要", self.app)
        self.assertIn("当前运行焦点", self.app)
        self.assertIn("运行明细", self.app)
        self.assertIn("默认只看队列、运行、复查和产物", self.app)
        self.assertIn('"workspaceHomeRuns"', self.app)
        self.assertIn("context.innerHTML = workspaceHomeContextMarkup", self.app)
        self.assertIn("上下文摘要", self.app)
        self.assertIn("当前建议", self.app)
        self.assertIn("默认只保留目标、资料、链路和运行四个信号", self.app)
        self.assertIn("资料明细", self.html)
        self.assertIn('"workspaceHomeContext"', self.app)
        self.assertIn(".workspace-agent-layer-bus", self.css)
        self.assertIn(".workspace-agent-layer-grid", self.css)
        self.assertIn(".workspace-agent-layer-node", self.css)
        self.assertIn("const schedulingRoot = $(\"workspaceCockpitScheduling\");", self.app)
        self.assertIn("schedulingRoot.innerHTML = workspaceSchedulingClosureMarkup(workspace, inputs, template, resources);", self.app)
        self.assertIn("resourcesPanel.innerHTML = workspaceHomeResourceSchedulingMarkup(workspace, formData, selectedWorkflowTemplate());", self.app)
        self.assertIn("workspaceHomeResourceSummaryMarkup(workspace, inputs, template, resources)", self.app)
        self.assertIn("workspaceSchedulingClosureMarkup(workspace, inputs, template, resources)", self.app)
        self.assertIn("workspaceCockpitResourceMatrixMarkup(workspace, { limit: 6 })", self.app)
        self.assertIn('"workspaceCockpitOperations", "workspaceHomeResources"', self.app)
        self.assertIn("renderWorkspaceHome();", self.app)
        self.assertIn('id="workspaceCockpitScheduling" class="workspace-cockpit-scheduling"', self.html)
        self.assertIn("数据 / 路径 / 环境 / GPU 调度闭环", self.app)
        self.assertIn("调度摘要", self.app)
        self.assertIn("当前资源焦点", self.app)
        self.assertIn("资源调度明细", self.app)
        self.assertIn('data-action="focus-scheduling-stage"', self.app)
        self.assertIn('data-target-id="${escapeHtml(targetId)}"', self.app)
        self.assertIn('$("workspaceCockpitScheduling")?.addEventListener("click"', self.app)
        self.assertIn('$("workspaceCockpitScheduling")?.addEventListener("keydown"', self.app)
        self.assertIn("focusWorkspaceExecutionNode(button.dataset.nodeId", self.app)
        self.assertIn("selectWorkspaceExecutionNode(nodeId, options = {})", self.app)
        self.assertIn(".workspace-scheduling-closure", self.css)
        self.assertIn(".workspace-scheduling-closure-rail", self.css)
        self.assertIn(".workspace-scheduling-closure-card", self.css)
        self.assertIn(".workspace-scheduling-closure-card[data-action]", self.css)
        self.assertIn(".workspace-focus-pulse", self.css)
        self.assertIn("@keyframes workspaceFocusPulse", self.css)
        self.assertIn("复现/部署总线", self.app)
        self.assertIn("Starter Chain 交接泳道", self.app)
        self.assertIn("Agent / Tool / AI · input_mapping / output_key / context", self.app)
        self.assertIn('id="workspaceCockpitPipeline" class="workspace-repro-pipeline"', self.html)
        self.assertIn('id="workspaceCockpitHandoff" class="workspace-cockpit-handoff"', self.html)
        self.assertIn('$(\"workspaceCockpitPipeline\")?.addEventListener(\"click\"', self.app)
        self.assertIn('$(\"workspaceCockpitHandoff\")?.addEventListener(\"click\"', self.app)
        self.assertIn('$(\"workspaceCockpitLayering\")?.addEventListener(\"click\"', self.app)
        self.assertIn("gapQueueRoot.innerHTML = workspaceAutomationGapQueueMarkup(workspace, formData, resources, template);", self.app)
        self.assertIn("运行蓝图", self.app)
        self.assertIn("数据/路径", self.app)
        self.assertIn("GPU 调度", self.app)
        self.assertIn("const chainRoot = $(\"workspaceCockpitChain\");", self.app)
        self.assertIn("const gapQueueRoot = $(\"workspaceCockpitGapQueue\");", self.app)
        self.assertIn("chainRoot.innerHTML = workspaceExecutionChainStructureMarkup(workspace, template);", self.app)
        self.assertIn("缺口 / 证据队列", self.app)
        self.assertIn(".workspace-gap-queue", self.css)
        self.assertIn(".workspace-gap-queue-list", self.css)
        self.assertIn('id="workspaceCockpitGapQueue" class="workspace-cockpit-gap-queue"', self.html)
        self.assertIn('$(\"workspaceCockpitGapQueue\")?.addEventListener(\"click\"', self.app)
        self.assertIn('$(\"workspaceCockpitChain\")?.addEventListener(\"click\"', self.app)
        self.assertIn("Starter Chain 执行结构", self.app)
        self.assertIn("当前实例执行结构", self.app)
        self.assertIn("执行链脊柱", self.app)
        self.assertIn("默认只看阶段进度", self.app)
        self.assertIn("阶段节点详情", self.app)
        self.assertIn("workspaceHomeExecutionRoadmapMarkup(workspace, selectedWorkflowTemplate())", self.app)
        self.assertIn("执行链责任矩阵", self.app)
        self.assertIn("workspaceHomeResponsibilityMatrixMarkup(workspace, selectedWorkflowTemplate(), { limit: 10 })", self.app)
        self.assertIn('data-action="open-workspace-node"', self.app)
        self.assertIn('if (action === "open-workspace-node" && button.dataset.nodeId)', self.app)
        self.assertIn('const active = node.id === state.selectedWorkspaceNodeId ? " active" : "";', self.app)
        self.assertIn('"workspaceHomeFlow"', self.app)
        self.assertIn('class="workspace-section workspace-home-flow-section"', self.html)
        self.assertIn(".workspace-home-responsibility-row", self.css)
        self.assertIn(".workspace-home-flow-section", self.css)
        self.assertIn('classList.toggle("home-active", next === "home")', self.app)
        self.assertIn(".workspace-project-modules.home-active", self.css)
        self.assertIn('const WORKSPACE_PROJECT_TABS = ["home", "project", "workflow", "chat", "agents", "tools", "model", "runs"];', self.app)
        self.assertIn("WORKSPACE_PROJECT_TABS.includes(snapshot.workspaceTab)", self.app)
        self.assertIn('saveStoredValue(STORAGE_KEYS.workspaceProjectConfigOpen, "1");', self.app)
        self.assertIn('"config": "project"', self.app)
        for tab_id in ["home", "project", "workflow", "chat", "agents", "tools", "model", "runs"]:
            self.assertIn(f'data-tab="{tab_id}"', self.html)
        self.assertIn("启动包", self.app)
        self.assertIn("启动模式影响", self.app)
        self.assertIn("Starter Chain 契约矩阵", self.app)
        self.assertIn("input_mapping / output_key / Agent / gate", self.app)
        self.assertIn("创建前确认快照、队列、门禁和资源动作", self.app)
        self.assertIn("不提交队列", self.app)
        self.assertIn("只允许源码、路径、数据、环境、GPU 和产物探测", self.app)
        self.assertIn("先发现，再通过 readiness gate 后完整运行", self.app)
        self.assertIn("readiness gate 后运行", self.app)
        self.assertIn("workspaceNodeIoContractState(node, node, { workspace: {}, template })", self.app)
        self.assertIn('action: "create-workspace"', self.app)
        self.assertIn('action: "create-workspace-discover"', self.app)
        self.assertIn('action: "create-workspace-run"', self.app)
        self.assertIn("Agent/Tool/AI", self.app)
        ...
        self.assertIn("数据集发现计划", self.app)
        self.assertIn("dataset_discovery", self.app)
        self.assertIn("workspaceDatasetDiscoveryPlanMarkup(datasetPlan", self.app)
        self.assertIn("部署计划", self.app)
        self.assertIn("deployment_plan", self.app)
        self.assertIn("workspaceDeploymentPlanMarkup(deploymentPlan", self.app)
        self.assertIn("smoke_test_command", self.app)
        self.assertIn("observe_commands", self.app)
        self.assertIn("stop_command", self.app)
        self.assertIn("复现脚本", self.app)
        self.assertIn("bundle.command_script", self.app)
        self.assertIn("copy-execution-bundle-script", self.app)
        self.assertIn("copy-execution-bundle-json", self.app)
        self.assertIn("复制包 JSON", self.app)
        self.assertIn("workspaceExecutionBundlePackageText(workspace, manifest)", self.app)
        self.assertIn("workspaceExecutionBundleScriptText(workspace, manifest)", self.app)
        self.assertIn("function workspaceExecutionBundleMissingAction", self.app)
        self.assertIn("function workspaceExecutionBundleMissingMarkup", self.app)
        self.assertIn("item.fix_action", self.app)
        self.assertIn("workspaceExecutionBundleMissingMarkup(item)", self.app)
        self.assertIn("workspaceHomeCurrentPlaybookItem(workspace, automation)", self.app)
        self.assertIn("workspaceHomeActionButtonMarkup(button", self.app)
        self.assertIn('"workspaceHomeCockpit"', self.app)
        self.assertIn('"workspaceHomePreflight"', self.app)
        self.assertIn('"workspaceHomeReadiness"', self.app)
        self.assertIn("automation?.preflight", self.app)
        self.assertIn("自动化预检矩阵", self.app)
        self.assertIn("data-tab", self.app)
        self.assertIn("switch-workspace-tab", self.app)
        self.assertIn("button.dataset.tab", self.app)
        self.assertIn("workspace-execution-bundle-script-text", self.app)
        self.assertIn("copy-workspace-run-script", self.app)
        self.assertIn("workspace-run-package", self.app)
        self.assertIn("脚本已归档", self.app)
        self.assertIn("执行包归档", self.app)
        self.assertIn("copy-node-execution-bundle-script", self.app)
        self.assertIn("workspace-node-execution-bundle-script", self.app)
        self.assertIn("data-workspace-help=\"${escapeHtml(copyHelp)}\"", self.app)
        self.assertIn('"run-workspace-to-selected-node": "运行到当前节点"', self.app)
        self.assertIn("auto_apply: false", self.app)
        self.assertIn("until_node_id: node.id", self.app)
        self.assertIn('data-action="run-workspace-to-selected-node"', self.app)
        self.assertIn("void runWorkspaceToNode(button.dataset.nodeId);", self.app)
        self.assertIn("交付契约", self.app)
        self.assertIn("workspaceDeliveryContractMarkup(deliveryContract", self.app)
        self.assertIn("workspaceExecutionBundleMarkup(manifest", self.app)
        self.assertIn("payload.execution_package", self.app)
        self.assertIn("执行包 ${executionPackage.status}", self.app)
        self.assertIn("bundle.next_action", self.app)
        self.assertIn("action.nodeId || action.node_id", self.app)
        self.assertIn("action.targetId || action.target_id", self.app)
        self.assertIn('data-target-id="${escapeHtml(targetId)}"', self.app)
        self.assertIn('data-tab="${escapeHtml(tab)}"', self.app)
        ...
        self.assertIn("编排契约", self.app)
        self.assertIn("workspace?.automation?.orchestration_contract", self.app)
        self.assertIn("workspaceOrchestrationContractMarkup(workspace", self.app)
        self.assertIn("workspaceCockpitResourceMatrixMarkup(workspace)", self.app)
        self.assertIn("调度决策", self.app)
        self.assertIn("workspaceSchedulerDecisionMarkup(scheduler)", self.app)
        self.assertIn('data-action="select-workspace-scheduler-candidate"', self.app)
        self.assertIn('data-action="apply-workspace-scheduler-candidate"', self.app)
        self.assertIn('data-server-id="${escapeHtml(serverId)}"', self.app)
        self.assertIn('data-gpu-index="${escapeHtml(gpuIndex)}"', self.app)
        self.assertIn('data-mode="${escapeHtml(candidate.mode || "gpu")}"', self.app)
        self.assertIn('data-gpu-policy="${escapeHtml(gpuPolicy)}"', self.app)
        self.assertIn('data-min-free-memory-gib="${escapeHtml(minFreeMemoryGib)}"', self.app)
        self.assertIn('data-node-id="${escapeHtml(gpuNode?.id || "")}"', self.app)
        self.assertIn("function workspaceSchedulerCandidateFromButton", self.app)
        self.assertIn("async function applyWorkspaceSchedulerCandidate", self.app)
        self.assertIn("scheduler_candidate: candidate", self.app)
        self.assertIn("apply_evidence: false", self.app)
        self.assertIn("force: true", self.app)
        self.assertIn('data-action="apply-workspace-backfill-item"', self.app)
        self.assertIn('data-action="focus-workspace-backfill-target"', self.app)
        self.assertIn('data-node-kind="${escapeHtml(item.node_kind || "")}"', self.app)
        self.assertIn("function workspaceBackfillItemFromButton", self.app)
        self.assertIn("function focusWorkspaceBackfillTarget", self.app)
        self.assertIn("workspaceBackfillFocus", self.app)
        self.assertIn("function workspaceDetailFieldFocusMarkup", self.app)
        self.assertIn('data-role="workspace-detail-field-focus"', self.app)
        self.assertIn("async function applyWorkspaceBackfillItem", self.app)
        self.assertIn("apply_defaults: false", self.app)
        self.assertIn("backfill_item: item", self.app)
        self.assertIn('if (action === "select-workspace-scheduler-candidate")', self.app)
        self.assertIn('if (action === "apply-workspace-scheduler-candidate")', self.app)
        self.assertIn('if (action === "apply-workspace-backfill-item")', self.app)
        self.assertIn('if (action === "focus-workspace-backfill-target")', self.app)
        self.assertIn("setProcessFocusForGpu(serverId, gpuIndex);", self.app)
        self.assertIn('focusWorkspaceExecutionNode(nodeId, { targetId: "workspaceExecutionDetail", tab: "home", block: "center" });', self.app)
        self.assertIn(".workspace-scheduler-candidate-main", self.css)
        self.assertIn(".workspace-scheduler-candidate-actions", self.css)
        self.assertIn(".workspace-scheduler-candidate-apply", self.css)
        self.assertIn(".workspace-backfill-item-actions", self.css)
        self.assertIn(".workspace-backfill-apply", self.css)
        self.assertIn(".workspace-backfill-locate", self.css)
        self.assertIn(".workspace-evidence-locate", self.css)
        self.assertIn(".workspace-detail-field-focus", self.css)
        self.assertIn("workspaceAutomationStateMachineMarkup(workspace)", self.app)
        self.assertIn("自动化状态机", self.app)
        self.assertIn("自动化剧本", self.app)
        self.assertIn("workspaceAutomationPlaybookMarkup(workspace", self.app)
        ...
        self.assertIn("Agent 实际执行", self.app)
        self.assertIn("data-agent-debug-execute-llm", self.app)
        self.assertIn("manageAgentDebugExecuteLlm", self.app)
        self.assertIn("execute_llm: Boolean(state.workspaceAgentDebug.executeLlm)", self.app)
        self.assertIn("execute_llm: Boolean(state.manageAgentDebug.executeLlm)", self.app)
    ...
    def test_cockpit_actions_explain_refresh_and_templates(self) -> None:
        ...
        self.assertIn("/styles.css?v=20260609cf", self.html)
        self.assertIn("/app.js?v=20260609cf", self.html)
        ...
        self.assertIn('const nextTab = ["templates", "agents", "tools", "ai"].includes(String(button.dataset.tab || "")) ? String(button.dataset.tab) : "";', self.app)
